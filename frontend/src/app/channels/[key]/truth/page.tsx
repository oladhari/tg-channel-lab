// frontend/src/app/channels/[key]/truth/page.tsx
"use client";

import Link from "next/link";
import { useParams } from "next/navigation";
import { useState } from "react";
import { getTruthGridSim, type GridSim } from "../../../../lib/api";
...
export default function ChannelTruthPage() {
  ...
  const [preferSeconds, setPreferSeconds] = useState(true);
  const [maxHoldSec, setMaxHoldSec] = useState(25 * 60);
  ...
  async function run() {
    ...
    setErr(null);
    try {
      const res = await getTruthGridSim({
        channel_key: channelKey,
        tp_values: tpValues,
        sl_values: slValues,
        start_balance_sol: start,
        entry_sol: entry,
        prefer_seconds: preferSeconds,
  ...
  if (!channelKey) {
    return (
      <main style={{ padding: 24, fontFamily: "system-ui, sans-serif" }}>
        ...
        <h1 style={{ marginBottom: 6 }}>Truth Simulation</h1>
  ...
  return (
    <main style={{ padding: 24, fontFamily: "system-ui, sans-serif" }}>
      ...
      <h1 style={{ marginBottom: 6 }}>Truth Simulation: {channelKey}</h1>
      <p style={{ marginTop: 0, color: "#555" }}>
        Run a TP/SL grid on cached CoinGecko OHLCV (<code>truth_ohlcv_cache</code>) for a fixed hold window.
        <br />
        Seconds cache is used first; fallback to minute. AMBIGUOUS outcomes are skipped.
      </p>
      ...
      <div style={{ border: "1px solid #ddd", borderRadius: 10, padding: 14, marginTop: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(240px, 1fr))", gap: 12 }}>
          ...
          <label style={{ display: "grid", gap: 6 }}>
            <span style={{ fontSize: 12, color: "#555" }}>Prefer seconds cache</span>
            <select value={String(preferSeconds)} onChange={(e) => setPreferSeconds(e.target.value === "true")}>
              <option value="true">true (seconds → minute)</option>
              <option value="false">false (minute only)</option>
            </select>
          </label>

          <label style={{ display: "grid", gap: 6 }}>
            <span style={{ fontSize: 12, color: "#555" }}>Max hold (sec)</span>
            <input
              type="number"
              step="60"
              value={maxHoldSec}
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 12, flexWrap: "wrap" }}>
          <button onClick={run} disabled={loading}>
            {loading ? "Running..." : "Run truth simulation"}
          </button>
      ...
      <section style={{ marginTop: 22 }}>
        ...
        {!data ? (
          <p style={{ color: "#666" }}>No results yet. Click “Run truth simulation”.</p>
        ) : (
